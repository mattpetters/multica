...
WHERE id = $1
RETURNING id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, archived_at, archived_by
`
...
func (q *Queries) ArchiveIssue(ctx context.Context, arg ArchiveIssueParams) (Issue, error) {
	row := q.db.QueryRow(ctx, archiveIssue, arg.ID, arg.ArchivedBy)
	var i Issue
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
SELECT id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, archived_at, archived_by FROM issue
WHERE id = $1
`
...
type ListArchivedIssuesParams struct {
	WorkspaceID pgtype.UUID `json:"workspace_id"`
...
func (q *Queries) ListArchivedIssues(ctx context.Context, arg ListArchivedIssuesParams) ([]ListArchivedIssuesRow, error) {
	rows, err := q.db.Query(ctx, listArchivedIssues,
		arg.WorkspaceID,
		arg.Limit,
		arg.Offset,
		arg.Status,
